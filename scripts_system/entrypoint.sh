...
if [ ! -n "${DST_CLUSTER_NAME:-}" ] then
    CLUSTER_NAME = "Cluster_1"
fi

DIR_MODS_SYS="/opt/dst_server/mods"
DIR_MODS_USER="${DST_USER_DATA_PATH}/DoNotStarveTogether/${CLUSTER_NAME}/mods"
FILE_CLUSTER_TOKEN="${DST_USER_DATA_PATH}/DoNotStarveTogether/${CLUSTER_NAME}/cluster_token.txt"
...
if [ "$1" == "dontstarve_dedicated_server_nullrenderer" ] || [ "$1" == "supervisord" ]; then
	...
	mkdir -p "${DST_USER_DATA_PATH}"
        ...
        touch "${DST_USER_DATA_PATH}/DoNotStarveTogether/${CLUSTER_NAME}/cluster_token.txt"
    fi
    ...
    if [ ! -f "${FILE_CLUSTER_TOKEN}" ]; then
        >&2 echo "Please fill in \`DoNotStarveTogether/${CLUSTER_NAME}/cluster_token.txt\` with your cluster token and restart server!"
        exit
    else
    ...
    if [[ -L "${DIR_MODS_SYS}" ]]; then
    	rm -f "${DIR_MODS_SYS}"
	cp -r "/opt/dst_default_config/DoNotStarveTogether/${CLUSTER_NAME}/mods" "${DIR_MODS_SYS}"
    fi
    ...
    if [ ! -d "${DIR_MODS_USER}" ]; then
        ...
        mkdir -p "${DST_USER_DATA_PATH}/DoNotStarveTogether/${CLUSTER_NAME}"
